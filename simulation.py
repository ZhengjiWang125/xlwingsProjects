...
# Monte Carlos Simulation with RunPython
...
def monte_carlos():
    ...
    plt.vlines(x = np.median(results[:,0]), ymin = 0, ymax = 1.0, linestyle = "--", label = "P50")
    plt.title('Simulation - Investment Multiple')
    plt.xlabel('Multiple',fontsize = 20)
    plt.legend(fontsize = 15)
    mc.pictures.add(fig, name = "Investment_Multiple", update = True,
                   left = mc.range("G10").left, 
                   top = mc.range("G10").top,
                   scale = 0.5)
